...
<div>
    <div class="nav_f">
        <input class="nav_f_input" type="button" value="批量删除" onclick='checkbox_this()'>
        <span id="num_show">0</span>
        <form>
            <input type="text" placeholder="菜名" id="student_query">
...
<div align="center" style="background-color: gainsboro;top: 136px;position: absolute;left: 389px; height: 70%" >
    <table align="center" border="1px" style="width: 755px;" >
        <tr>
            <th>
    ...
    <form action="showMenu" method="get" style=" position: relative;top:21px;" >
        <table align="center">
            <tr>
                <td>
                    ...
                    <c:if test="${pageInfo.pageNo != 1}">
                        <a class="mya" href="showMenu?pageNo=1">首页</a>
                    </c:if>
                </td>
                <td>
                    <c:if test="${pageInfo.pageNo == 1}">
                    ...
                    <c:if test="${pageInfo.pageNo != 1}">
                        <a class="mya" href="showMenu?pageNo=${pageInfo.pageNo - 1}">上一页</a>
                    </c:if>
                </td>
                ...
                <td>
                    ...
                    <c:if test="${pageInfo.pageNo != pageInfo.pageCount}">
                        <a class="mya" href="showMenu?pageNo=${pageInfo.pageNo + 1}">下一页</a>
                    </c:if>
                </td>
                <td>
                    <c:if test="${pageInfo.pageNo == pageInfo.pageCount}">
                    ...
                    <c:if test="${pageInfo.pageNo != pageInfo.pageCount}">
                        <a class="mya" href="showMenu?pageNo=${pageInfo.pageCount}">尾页</a>
                    </c:if>
                </td>
    ...
    <script>
            ...
            if (confirm('您是否确认删除？') === true) {
                $.ajax({
                    ...
                    success: function (data) {
                        if (data !== 0) {
                            alert("删除成功"); window.location.href="showMenu?pageNo=${pageInfo.pageNo}";//******
                        } else {
                            alert("网络拥堵，请稍后再试");
                        }
                    }
            ...
            if (count!==0&&confirm('您是否确认删除？\n删除后将无法恢复!') === true){
                ...
                $.ajax({
                    ...
                    success:function (data) {
                        if (data === "0") {
                            alert("删除成功");
                            window.location.href="showMenu?pageNo=${pageInfo.pageNo}";
                        } else {
        ...
        function query_fun() {
            ...
            if (tiaojian === ""){
                ...
                window.location.href="showMenu?pageNo=${pageInfo.pageNo}&tiaojian="+tiaojian+"&cut="+cut;
            }
        ...
        for (let i =0;i<aH.length;i++){
            let sts=aH[i].getAttribute('href')+"&tiaojian="+tiaojian+"&cut="+cut;
            aH[i].href=sts;
        }
        function pageBut(pagNo) {
            window.location.href="showMenu?pageNo="+pagNo+"&tiaojian="+tiaojian+"&cut="+cut;
        }
        function clear_form(){
            $("#student_query").val("");
